...
app.get('/rooms/:id/getPhotosByRoomID', async (req, res) => {
  try {
    const response = await axios.get(` http://3.21.180.150:5005/${req.params.id}/getPhotosByRoomID`);
...
app.get('/users/:id', async (req, res) => {
  try {
    const response = await axios.get(`http://3.143.95.170:5007/${req.params.id}`);
    res.send(response.data);
  } catch (err) {
